...
public class HostDynamicWorkload extends Host {
	...
	public double updateVmsProcessing(double currentTime) {
		...
		for (Vm vm : getVmList()) {
			// chiyuan's 
			Host host_condition = vm.getHost();

			if (true) {//host_condition != null) {
				...
				if (!Log.isDisabled()) {
					if (vm != null && vm.getHost() != null) {
						...
						System.out.println(totalAllocatedMips);
						System.out.println(totalRequestedMips);
						//if (totalRequestedMips == 0) {
						//	System.out.println(vm);
						//throw new RuntimeException("Intentional failure at line 3.");
						//}
						//else {
